...
class Voice_Sizer():
    ...
    def text_to_speech(self, text, file_name='assets/output.mp3', play_audio=True):
...
if __name__ == '__main__':
    ...
    keys = [
        os.getenv('BING_MAPKEY'),
        os.getenv('BING_SPEECHKEY'),
        os.getenv('LUIS_APPID'),
        os.getenv('LUIS_APPKEY'),
        os.getenv('GSHEET_SSKEY')
    ]

    if None in keys:
        print("Key Error: Please ensure the following env vars are set: 'BING_MAPKEY', 'BING_SPEECHKEY', 'LUIS_APPID', 'LUIS_APPKEY', 'GSHEET_SSKEY'")
    else:
        Voice_Sizer(BING_MAPKEY, BING_SPEECHKEY, LUIS_APPID, LUIS_APPKEY, gSheet_SSKEY)